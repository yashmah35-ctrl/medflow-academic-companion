                        ...
                        {selectedKholle?.format === "QIM" ? (
                          <div className="flex gap-1">
                            <Button type="button" size="sm" variant={p.isCorrect ? "default" : "outline"} className="h-7 px-2 text-xs"
                              onClick={() => { const updated = [...propositions]; updated[idx] = { ...updated[idx], isCorrect: true }; setPropositions(updated); }}>V</Button>
                            <Button type="button" size="sm" variant={!p.isCorrect ? "destructive" : "outline"} className="h-7 px-2 text-xs"
                              onClick={() => { const updated = [...propositions]; updated[idx] = { ...updated[idx], isCorrect: false }; setPropositions(updated); }}>F</Button>
                          </div>
                        ) : (
                          <input type="checkbox" checked={p.isCorrect}
                            onChange={() => { const updated = [...propositions]; updated[idx] = { ...updated[idx], isCorrect: !updated[idx].isCorrect }; setPropositions(updated); }}
                            className="h-4 w-4 rounded border-border accent-primary" />
                        )}
                        <span className="text-sm font-medium text-muted-foreground w-6">{p.id}.</span>
                        <Input value={p.text}
                          onChange={(e) => { const updated = [...propositions]; updated[idx] = { ...updated[idx], text: e.target.value }; setPropositions(updated); }}
                          placeholder={`Proposition ${p.id}`} className="flex-1" />
                      </div>
                      <Input value={p.explanation || ""}
                        onChange={(e) => { const updated = [...propositions]; updated[idx] = { ...updated[idx], explanation: e.target.value || undefined }; setPropositions(updated); }}
                    ...
                    <div key={p.id} className="space-y-1">
                      <div className="flex items-center gap-2">
                        {selectedKholle?.format === "QIM" ? (
                          <div className="flex gap-1">
                            <Button type="button" size="sm" variant={p.isCorrect ? "default" : "outline"} className="h-7 px-2 text-xs"
                              onClick={() => { const updated = [...editPropositions]; updated[idx] = { ...updated[idx], isCorrect: true }; setEditPropositions(updated); }}>V</Button>
                            <Button type="button" size="sm" variant={!p.isCorrect ? "destructive" : "outline"} className="h-7 px-2 text-xs"
                              onClick={() => { const updated = [...editPropositions]; updated[idx] = { ...updated[idx], isCorrect: false }; setEditPropositions(updated); }}>F</Button>
                          </div>
                        ) : (
                          <input type="checkbox" checked={p.isCorrect}
                            onChange={() => { const updated = [...editPropositions]; updated[idx] = { ...updated[idx], isCorrect: !updated[idx].isCorrect }; setEditPropositions(updated); }}
                            className="h-4 w-4 rounded border-border accent-primary" />
                        )}
                        <span className="text-sm font-medium text-muted-foreground w-6">{p.id}.</span>
                        <Input value={p.text}
                          onChange={(e) => { const updated = [...editPropositions]; updated[idx] = { ...updated[idx], text: e.target.value }; setEditPropositions(updated); }}
                          placeholder={`Proposition ${p.id}`} className="flex-1" />
                      </div>
                      <Input value={p.explanation || ""}
                        onChange={(e) => { const updated = [...editPropositions]; updated[idx] = { ...updated[idx], explanation: e.target.value || undefined }; setEditPropositions(updated); }}
                        placeholder={`Explication ${p.id} (optionnel)`} className="ml-8 text-xs h-7" />
                    </div>
                  ))}
                </div>
              ...
              return (
                <div key={p.id} className="space-y-0">
                  <button onClick={() => toggleAnswer(currentQuestion.id, p.id)}
                    ...
                    <span className="flex-1 text-sm text-foreground">{p.text}</span>
                    {showResults && (
                      p.isCorrect ? <CheckCircle2 className="h-5 w-5 text-green-500" />
